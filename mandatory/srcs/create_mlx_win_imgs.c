...
/*   Updated: 2023/05/29 20:24:59 by jeojeon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/cub3d.h"

static void	create_texture_imgs(t_info *const info)
{
	info->objects.north_wall.ptr = \
	mlx_xpm_file_to_image(info->sys.mlx_ptr, info->objects.north_wall.file, \
	&(info->objects.north_wall.rows), &(info->objects.north_wall.cols));
	free(info->objects.north_wall.file);
	info->objects.north_wall.file = NULL;
	info->objects.south_wall.ptr = \
	mlx_xpm_file_to_image(info->sys.mlx_ptr, info->objects.south_wall.file, \
	&(info->objects.south_wall.rows), &(info->objects.south_wall.cols));
	free(info->objects.south_wall.file);
	info->objects.south_wall.file = NULL;
	info->objects.west_wall.ptr = \
	mlx_xpm_file_to_image(info->sys.mlx_ptr, info->objects.west_wall.file, \
	&(info->objects.west_wall.rows), &(info->objects.west_wall.cols));
	free(info->objects.west_wall.file);
	info->objects.west_wall.file = NULL;
	info->objects.east_wall.ptr = \
	mlx_xpm_file_to_image(info->sys.mlx_ptr, info->objects.east_wall.file, \
	&(info->objects.east_wall.rows), &(info->objects.east_wall.cols));
